...
type FormData = NoUndefinedField<Pick<Schema, 'name'>>
const nameShema = schema.pick(['name'])

const MAX_PURCHASES = 5

export default function MainHeader() {
  // sử dụng hook useQueryConfig() đã được custom
  ...
  const onSubmitSearch = handleSubmit((data) => {
    ...
    // nếu có order -> omit 2 trường order và sort_by
    const configSearch = queryConfig.order
      ? omit(
          {
            ...queryConfig,
            name: data.name
    ...
    navigate({
      ...
      search: createSearchParams(configSearch).toString()
    })
  })
  ...
  // hiển thị các sp được add vào giỏ hàng
    ...
    <div className='bg-[linear-gradient(-180deg,#f53d2d,#f63)] pb-5 pt-2 text-white'>
      <div className='container'>
        <div className='flex justify-end'>
          {/* Popover cho: World + Tiếng Việt + Mũi Tên */}
          <Popover
            // className, renderPopover
            className='flex cursor-pointer items-center py-1 hover:text-gray-300'
            renderPopover={
              // hold chuột vào -> hiện 2 mục: Tiếng Việt, English
              <div className='relative rounded-sm border border-gray-200 bg-white shadow-md'>
                <div className='flex flex-col px-3 pl-3 pr-28'>
          ...
          {/* Popover cho: Avatar + tên tk */}

          {isAuthenticated && (
            <Popover
              className='ml-6 flex cursor-pointer items-center py-1 hover:text-gray-300'
              renderPopover={
                // hold chuột vào tên TK -> hiện 3 mục: Tài khoản của tôi, Đơn mua, Đăng xuất
                <div className='relative rounded-sm border border-gray-200 bg-white shadow-md'>
                  <Link
                    to={constPath.profile}
                    className='hover: hover:text-orange-500 block w-full bg-slate-100 bg-white px-4 py-3 text-left'
                  >
                    Tài khoản của tôi
                  </Link>
                  <Link
            ...
            >
              ...
              <div className='mr-2 h-6 w-6 flex-shrink-0'>
                <img
                  src='https://down-vn.img.susercontent.com/file/sg-11134004-23030-4jyc2n8lseov0c_tn'
                  alt='avatar'
          ...
          {/* nếu chưa login -> show 2 link */}
          {!isAuthenticated && (
            <div className='flex items-center'>
              <Link to={constPath.register} className='mx-3 capitalize hover:text-white/70'>
                Đăng ký
              </Link>
              <div className='h-4 border-r-[1px] border-r-white/40' />
              <Link to={constPath.login} className='mx-3 capitalize hover:text-white/70'>
                Đăng nhập
              </Link>
            </div>
          )}
          ...
          <div className='col-span-1 justify-self-start'>
            ...
            >
              <Link to='/' className='relative'>
                ...
                <span className='absolute left-[10] top-[-5px] rounded-full bg-white px-[9px] py-[1px] text-xs text-orange'>
